...
void StartTcpClientTask(void const *argument)
{
	...
	void *data;
	u16_t len;
	u16_t read; //read buffer index
	u16_t written; //write buffer index

	LWIP_UNUSED_ARG(argument);

	while(1)
	{
		read = 0; //clear indexes
		written = 0;
		...
		if (conn != NULL)
		{
			...
			if(err != ERR_OK)
			{
				...
				osDelay(1000); //wait 1sec before reconnect
				continue;
			}
			...
			do
			{
				if(netconn_write_partly(conn, (const void*)(&packet + written), (sizeof(struct time_packet) - written), NETCONN_NOFLAG, (size_t*)&len) != ERR_OK)
				{
			...
			}while(written < sizeof(struct time_packet)); //send request
